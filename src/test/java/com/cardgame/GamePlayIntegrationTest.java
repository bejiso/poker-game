...
public class GamePlayIntegrationTest {
    ...
    public void gameDeterministicPlayTest() throws Exception {
        ...
        getCardsOfPlayer(gameId,
                player1Id,
                "[{\"faceValue\":\"ACE\",\"suit\":\"HEARTS\",\"value\":1}," +
                        "{\"faceValue\":\"TWO\",\"suit\":\"HEARTS\",\"value\":2}," +
                        "{\"faceValue\":\"THREE\",\"suit\":\"HEARTS\",\"value\":3}]");

        dealCardsToPlayer(gameId, player2Id, 3);
        getCardsOfPlayer(gameId,
                player2Id,
                "[{\"faceValue\":\"FOUR\",\"suit\":\"HEARTS\",\"value\":4}," +
                        "{\"faceValue\":\"FIVE\",\"suit\":\"HEARTS\",\"value\":5}," +
                        "{\"faceValue\":\"SIX\",\"suit\":\"HEARTS\",\"value\":6}]");

        getGameScores(gameId, "[{\"playerId\":1,\"score\":15}," +
                "{\"playerId\":0,\"score\":6}]");
    }

    private void getGameScores(String gameId, String expectedScores) {
        WebTestClient.ResponseSpec responseSpec = webClient.get().uri("/game/" + gameId + "/scores")
                .exchange()
                .expectStatus()
                .isEqualTo(200);
    ...
    private void getCardsOfPlayer(String gameId, String playerId, String expectedResponse) {
        WebTestClient.ResponseSpec responseSpec = webClient.get().uri("/game/" + gameId + "/player/" + playerId + "/cards")
                .exchange()
                .expectStatus()
                .isEqualTo(200);
    ...
    private void dealCardsToPlayer(String gameId, String playerId, int cardsAmount) {
        webClient.put().uri("/game/" + gameId + "/player/" + playerId + "/deal?cards-amount=" + cardsAmount)
                .exchange()
                .expectStatus()
                .isEqualTo(200);
    }
    ...
    private String addPlayerToGame(String gameId) {
        WebTestClient.ResponseSpec responseSpec = webClient.put().uri("/game/" + gameId + "/player")
                .exchange()
                .expectStatus()
                .isEqualTo(200);
    ...
    private String respToString(WebTestClient.ResponseSpec responseSpec) {
        return new String(responseSpec.expectBody().returnResult().getResponseBody(), StandardCharsets.UTF_8);
    }

    private void addDeckToGame(String gameId, String deckId) {
        webClient.put().uri("/game/" + gameId + "/deck/" + deckId)
                .exchange()
                .expectStatus().isEqualTo(200);
    }

    private String createDeck() throws Exception {
        WebTestClient.ResponseSpec responseSpec = webClient.put().uri("/deck")
                .exchange()
                .expectStatus().isEqualTo(200);
        return respToString(responseSpec);
    }
    ...
    private void accessDeletedGameScores(String gameId) throws Exception {
        webClient.get().uri("/game/" + gameId + "/scores")
                .exchange()
                .expectStatus().isEqualTo(500)
                .expectBody().jsonPath("message").isEqualTo("game " + gameId + " doesn't exist");
    }


    private void deleteGameWithId(String gameId) throws Exception {
        webClient.delete().uri("/game/" + gameId)
                .exchange()
                .expectStatus().isEqualTo(200);
    }

    private String createGame() throws Exception {
        WebTestClient.ResponseSpec responseSpec = webClient.put().uri("/game")
                .exchange()
                .expectStatus().isEqualTo(200);
        return respToString(responseSpec);
    }
